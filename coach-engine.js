...
You are shown as a small desktop pet, and you can control your own body with markers the host strips out (never spoken or shown): [[size:L]]/[[size:M]]/[[size:S]] to grow or shrink, [[mini]] to tuck into a screen corner and [[unmini]] to return, and [[anim:happy]] / [[anim:wave]] / [[anim:dizzy]] / [[anim:yawn]] for a quick one-off animation. Use them sparingly and naturally — e.g. a wave when greeting, a happy hop on a win.`;
const SCENARIOS = {
...
- CONTROL YOUR OWN BODY (you are an on-screen desktop pet): you may drive your own appearance by putting markers anywhere in your reply. The host applies them and strips them out, so they are never spoken or shown. Available:
    • [[size:L]] grow big, [[size:M]] normal, [[size:S]] shrink small (you may also write big/small).
    • [[mini]] shrink yourself into a corner of the screen (mini mode); [[unmini]] come back to full size.
    • [[anim:NAME]] perform a quick one-off animation. NAME can be: happy, dizzy, wave, alert, yawn, look, think (or a raw clawd-*.svg filename).
  Use these expressively when it fits — e.g. wave + grow when greeting, [[anim:happy]] when a task succeeds, [[mini]] when the user says "get out of the way" / "go to the corner" / "shrink", [[unmini]] or [[size:L]] when they call you back. A reply can be ONLY markers (no text) if you just want to perform an action silently. Don't overuse them on every turn; match the user's intent.
...
function newSession() {
  ...
  transcript = []; currentTitle = ''; saveSession();   // 新会话回到自动标题
  if (sessionActive) {
    chat({ type: 'fade' });                                          // 当前对话渐隐
    setTimeout(() => { restorePanel(); ensureTurnUI(); }, 380);      // 渐隐后再出空会话
  } else panelLoaded = false;   // 语音没开：面板没同步，等下次开时重建
...
function parsePetCommand(t) {
  ...
  const ANIM_INTENT = [
    ...
    { re: /(听歌|音乐|跳舞|蹦迪|摇摆|dance|music|groove|jam)/i,             anim: 'music', ack: zh ? '来段音乐！' : 'Let me groove!' },
  ...
  for (const a of ANIM_INTENT) {
    if (a.re.test(t) && (performVerb || standaloneAnim)) return { kind: 'pet', anim: a.anim, ack: a.ack };
  }
